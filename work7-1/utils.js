...
function bubbleSort(str) {    /////////////////////////////
 ...
 //把比较前的数组状态和定位指针——————??????————保存到sortLog中，为保持演示效果，加多两行。
         ...
         if (arr[j] > arr[j + 1]) {
            ...
            //把比较之后的数组状态和定位指针保存到sortLog............._____________________
            sortLog.push([arr.concat(), [j, j + 1]])
         }
      }
   }
   //______
//把排序最终状态的数组和取消定位指针保存到sortLog中，因为动画的最后状态是不显示比较数据
...
/*
   sortLog.push([arr.concat(), [-1, -1]])
   //调试时请认真分析sortLog的数据结构 // console.log(sortLog)
...
function showLog(logValues, showElement) {
   ...
   //console.log(row)          ///console.log(pos)————————这都是什么鬼，wctm这怎么写的啊！
...
function insertSort(str) {
   ...
   let sortLog = []
   //比较轮数，数组有多少个数字，就比较数组长度-1轮
   for (let i=1; i<arr.length;++i) { //length不减1，7的话，下标为直接为7？？？？为什么
      //从第一个开始比较相邻的两个数值，
      for (let j=i; j>0; j--) {
